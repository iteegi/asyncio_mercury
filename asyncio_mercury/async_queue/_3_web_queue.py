...
async def process_order_worker(worker_id: int, queue: Queue):
    """Select an order from the queue and process."""
    while True:
...
async def place_order(request: Request) -> Response:
    """Put the order in the queue and reply to the user immediately."""
...
async def create_order_queue(app: Application):
    """Create a queue for 10 elements and 5 worker tasks."""
